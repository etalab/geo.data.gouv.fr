...
describe('<Download />', () => {
  ...
  describe('featureType', () => {
    it('should create a link to download wfs-featureType dataset type', () => {
      const link = 'https://inspire.data.gouv.fr/api/geogw/services/556c6066330f1fcd48338831/feature-types/drac:bretagne_immeuble_mh/download?format=GeoJSON&amp;projection=WGS84'
      const wrapper = shallow(<Download distribution={featureType} dlFormat={format}/>)
      expect(wrapper.html()).to.contain(link)
    })
  })

    describe('file-package', () => {
      it('should create a link to download file-package dataset type', () => {
        const link = 'https://inspire.data.gouv.fr/api/geogw/file-packages/e2880991300be9f2f4aa9f8bbcd629ea94501a72/N_AC1_GENERATEUR_SUP_S_032.TAB/download?format=GeoJSON&amp;projection=WGS84'
  ...
  describe('Unavailable dataset', () => {
    it('should display only dataset name', () => {
      ...
      expect(wrapper.html()).to.contain(
        '<select disabled=""><option>Télécharger</option>'
      )
    })
  })
